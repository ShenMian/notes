...
| 名称        | 描述                                                   | 包名                       |
| ----------- | ------------------------------------------------------ | -------------------------- |
| [alacritty] | 终端模拟器, 支持 GPU 加速的, 跨平台. 🦀                  | `alacritty` (pacman/scoop) |
...
| [Source Han Serif (思源宋体)] | 开源, 免费, 粗体.   | `adobe-source-han-serif-cn-fonts` (pacman) / `source-han-serif-sc` (scoop) |
| [Nerd Font]                   | 字元图标.           |                                                                            |
...
| [Mindustry]     | 塔防+RTS              | `com.github.Anuken.Mindustry` (flatpak) |
| [Cataclysm-DDA] | 回合制, 开源, 跨平台. | `cataclysm-dda` (pacman)                |
| [OpenTTD]       | 交通运输模拟.         | `openttd` (pacman) / `openttd` (scoop)  |
...
| [tldr]           | man 命令扩展, 以例子的形式展示命令用法.      | `tldr` (pacman)                     |
...
详情请参考[大语言模型使用指南](LLM_入门指南.md#mobile).
