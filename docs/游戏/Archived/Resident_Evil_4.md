# Resident Evil 4

**别名**: Biohazard 4.  
**中文**: 生化危机 4.

## 主线故事

![Ashley Graham](assets/resident_evil_4.webp)

### 第 1 章

密码线索位于二楼的一本书里.

![衣柜密码线索](assets/wardrobe.webp)

**村长宅邸 1F 衣柜密码**: 谷物, 猪彘, 孩童.

### 第 4 章

大小洞窑祭坛的密码为附加岩壁上的黄色涂鸦, 无输入顺序.

**大洞窑祭坛密码**: 1 点钟方向, 11 点钟方向, 6 点钟方向.

![大洞窑祭坛密码](assets/large_cave_shrine.webp)

**小洞窑祭坛密码**: 9 点钟方向, 7 点钟方向, 5 点钟方向.

![小洞窑祭坛密码](assets/small_cave_shrine.webp)

https://www.pcgamer.com/resident-evil-4-remake-cave-puzzle/

### 第 7 章

这个谜题单纯就是怎么合理怎么摆.

**珍藏室**: 铁剑, 金剑, 血剑, 锈剑.

![珍藏室谜题答案](assets/collection_room.webp)

## 逆命殊途 (Separate Ways)

![Ada Wong](assets/ada.webp)

### 第 1 章

与主线故事中第 4 章的解密方式相同.

**密码**: 4 点钟方向, 9 点钟方向, 12 点钟方向.

![密码](assets/ada_puzzle.webp)

### 第 6 章

通过观察指纹所在的位置, 可以确定三个数字. 然后通过指纹完整度判断顺序, 从完整到不完整.

由于密码门并不与具体的命名区域相关联, 下面按照出现顺序给出:

- **第一扇密码门密码**: 148E.
- **第二扇密码门密码**: 237E.
- **第三扇密码门密码**: 430E.

## 收集品

16 个发条城堡位置: <https://www.pcgamer.com/resident-evil-4-remake-clockwork-castellan-locations-salazar/>

## 贴士

- 一周目最大手提箱容量为 8x13, 二周目最大容量为 9x13.

## BOSS

- 湖怪跃出水面, 从正面砸向小船: 往右划船.
- 湖怪从侧面游向小船: 张嘴时攻击口部. 若失误, 则准备全力快速点击 `F` 键.
