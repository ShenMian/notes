...
- **远程 Shell**: 利用了杀毒软件对 DOS 批处理脚本不敏感的缺陷, 实现了可以绕过杀毒软件的远程 Shell. 为了不依赖第三方可执行文件, 通讯使用的是系统自带的 FTP 命令.
...
视频是使用 PPT+实践的方式来进行教学的, PPT 讲解完就编写代码并展示执行结果, 内容十分紧凑, 毫不拖沓.  
...
- n + 1 问题:
    ...
    C++ 兼容大部分 C 代码本身是一件好事, 但这却导致部分代码 C 和 C++ 代码混用的情况, 使得代码风格迥异.
...
- 依赖管理困难:
    ...
    虽然存在诸如 [vcpkg] 和 [conan] 这样的第三方包管理器, 但在使用过程中我仍然频繁地遇到依赖项构建错误, 导致项目无法继续编译.  
...
- Python: 相比 C 语言, 我觉得 Python 是一门语法十分复杂的语言. 因为之前运行 Python 脚本经常遇到报错, 我对 Python 的印象并不好. 再加上我之前使用 C++ 开发过的项目, 很多无法使用 Python 来实现, 因此我没有选择这个语言.
- Java: 学习了 C++ 后再回头看 Java, 我觉得 Java 的代码有大量的冗余, 并不美观. 比如喜欢使用单词全拼而非缩写, C++ 则倾向用符号来表示, 十分简洁. 在我看来, 对于经验丰富的开发者来说, 使用冗长的全拼单词是多余的.
