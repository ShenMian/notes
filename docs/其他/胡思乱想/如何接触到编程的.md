...
- 依赖管理困难:
  ...
  相比部分更现代的编程语言, C++ 没有自己的包管理器. 库之间的调用更像是直接把代码混合在一起, 使得添加依赖项需要涉及构建系统.  
  使用较广泛的第三方包管理器有 vcpkg 和 conan. 我在编写游戏引擎的时候使用过这两个工具, 使用过程中, 频繁的出现依赖项构建出错导致项目无法进一步编译.  
  我尝试向包管理器提交问题, 最终问题也有被修复, 但耗时可能很长.  
  构建上的错误导致游戏引擎的开发不断停止, 我最终甚至尝试混合使用 vcpkg 和 conan. 因为这样可以把单个依赖项在单个包管理器中出故障的概率降低为单个依赖项在两个包管理器中出故障的概率, 这几乎可以解决包管理器自身导致的错误, 但对于依赖项本身存在的问题却无法解决.
