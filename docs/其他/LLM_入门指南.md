...
本文不涉及技术细节, 专门介绍**如何使用 LLM**, 帮助读者快速上手.
...
![Cherry Studio 添加知识库](assets/cherry_add_knowledge_base.webp){ width=50% style="display: block; margin: 0 auto" }  
...
![Cherry Studio 知识库添加网站地图](assets/cherry_add_sitemap.webp){ width=70% style="display: block; margin: 0 auto" }  
...
[granite-4-h-tiny]: https://huggingface.co/ibm-granite/granite-4.0-h-tiny
...
词元是 LLM 处理文本的基本单位[^token], 也是 API 计费使用到的单位. 其与自然语言文本的换算关系大致为:

- 1 个英文字符 $\approx$ 0.3 词元.
- 1 个中文字符 $\approx$ 0.6 词元.

部分 LLM 客户端会显示精确的词元数, 方便用户计算对话消耗的费用. 如果需要自行计算词元数量, 则需要使用到相应模型的分词器 (tokenizer)[^tokenizer].
