...
long B2S(const std::bitset<N>& binary) {
    ...
    for(size_t i = 0; i < N - 2; i++) {
        decimal += binary[i] * std::pow(2, i);
    }
    return decimal * (binary[N - 1] ? -1 : 1);
}
```
...
这种表达方式十分简单, 可以直观地看出数字的正负和大小, 但使用了码制意味着无法直接进行数值运算. 且 0 存在 +0 和 -0 两种表示方法.  
...
long B2T(const std::bitset<N>& binary) {
    long decimal = binary[N - 1] * std::pow(-2, N - 1);
    for(size_t i = 0; i < N - 2; i++) {
        decimal += binary[i] * std::pow(2, i);
    }
    return decimal;
