...
- **[RikkaHub]**: 开源 (AGPLv3), 支持 MCP, 在线搜索. (类似移动端的 Cherry Studio)
- **[PocketPal]**: 开源 (MIT), **仅支持离线模型**. (类似移动端的 LM Studio)
- **[MNN Chat]**: 开源 (Apache-2.0), **仅支持离线模型**, 支持 iOS (需要自行编译), 支持许多模型 (包括但不限于 Qwen3/DeepSeek), 有会话记录.
- **[Edge Gallery]**: 开源 (Apache-2.0), **仅支持离线模型**, 支持最新的 Gemma 3n 系列模型, 无法保持会话记录.
