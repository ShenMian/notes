...
相比传统的基于文本的搜索引擎, 基于向量数据库的 RAG 拥有语义检索的能力.

除了生成回答的 LLM 需要借助两种模型:
...
下面将以以 Cherry Studio 为例, 描述如何创建一个关于 wgpu 的知识库:
...
找到 wgpu 的文档网站后, 通过 <https://www.xml-sitemaps.com> 等工具生成 Sitemap, 里面包含了该网页下所有子页面的 URL. 然后将 Sitemap 的 URL 添加到 Cherry Studio 中.
