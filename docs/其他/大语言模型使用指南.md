...
LM Studio 会预估模型是否能在本地运行, 尽量选择 `Full GPU Offload Possible` 的, 这样可以将模型完整的加载进显存, 使得推理效率最大化. `Partial GPU Offload Possible` 的通常可以运行, 但是效率会显著降低. 值得注意的是, 与模型进行对话也会增加内存使用量, 所以并非能加载就能正常使用.
