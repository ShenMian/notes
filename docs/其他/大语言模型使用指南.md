...
- **[LM Studio]**: 跨平台, 闭源免费, **仅支持离线模型**, 界面简洁易用. 支持图片/文档解析, Flash Attention.
...
![Cherry Studio 提供的内置 MCP 服务器](assets/cherry_mcps.png){ width=80% style="display: block; margin: 0 auto" }  

后续便可用通过下面方式在会话中启用 MCP:

![启用 MCP](assets/cherry_mcp_servers.png){ width=60% style="display: block; margin: 0 auto" }  
