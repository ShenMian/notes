...
| 命令                           | 描述                           | 备注                                                                                     |
|--------------------------------|------------------------------|------------------------------------------------------------------------------------------|
| `git stash --keep-index`       | 贮藏除了暂存区的内容           | 可以用于测试暂存区中准备提交的内容是否完整有效, 比如是否能通过编译                       |
...
- **大文件按需下载**: 被 LFS 跟踪的文件将被存储在 LFS 服务器上, Git 仓库中指保留一个轻量的指针. 这使得 Git 仓库的体积与大文件无关, 可以仅在执行检出操作时下载必要的大文件. 这方面有点类似 Subversion (SVN).
...
在执行部分 Git 命令 (如 `git gc`) 时, 这些松散对象会通过**增量压缩**被打包为包文件 (packfile), 存储在 `objects/pack/` 目录下.
...
在打包过程中, 相似的对象的一方会引用另一方, 仅记录它们之间的差异, 从而节省空间. 引用关系存储在 `.idx` 文件中.
