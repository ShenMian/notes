...
反坦克小组通常具有较高的隐蔽基础值 (1.75), 且部署在开阔区域的建筑内 (3.5), 以便能利用隐蔽和射程优势, 防止坦克推进.

以 TOW-2B 为例, 下表是不同距离下导弹击中目标的时间, 也是被攻击载具的反应时间:
...
- 主炮射程为 1.4 km. 如果有敌方单位视野, 瞄准+开火时间长达 3-4 秒.

在通过 LoS 工具确定敌方单位所在建筑物视野后, 利用机动性快速接近建筑物, 使其进入机枪的射程.

若确定对方反坦克小组无补给后 (如已经进行了一轮火炮打击), 也可以在远处诱导对方攻击, 消耗其弹药 (4-6 发).

根据 ATGM 飞行方向反推对方反坦克单位具体位置:
- 使用火炮打击其最后位置.
- 如果在己方载具位于烟幕内且敌方单位位于射程内, 可以使用 "强制射击" 命令攻击其最后位置.
