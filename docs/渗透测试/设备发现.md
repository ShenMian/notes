...
Metasploit 包含若干[^1]用于设备发现的模块. 相比其他工具, 它们使用起来更加繁琐. 扫描结果会被保存到工作区中.

[^1]: 截止 2025 年 4 月, 一共只有 3 个模块. 包含 ARP 扫描.
