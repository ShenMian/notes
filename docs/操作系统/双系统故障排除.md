...
临时解决办法是让 Windows **重启**, 然后在重启时直接切换至 Linux.

## 时间不一致

由于主板上的硬件时钟 (Real-Time Clock, RTC) 不包含时区信息, 默认情况下, Linux 会将 RTC 视为 UTC 时间, 而 Windows 则将其视为本地时间.  
因此, 只要本地时间与 UTC 时间存在偏差, 就会造成双系统时间不一致.
...
```sh
sudo dislocker -V /dev/sdXn -p<RECOVERY_KEY> -- /mnt/bitlocker         # 通过恢复密钥解密
sudo dislocker -V /dev/sdXn -u -- /mnt/bitlocker                       # 通过用户密码解密
sudo mount -t ntfs /mnt/bitlocker/dislocker-file /run/media/$USER/data # 挂载
```
