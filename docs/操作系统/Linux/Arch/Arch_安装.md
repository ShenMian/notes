...
- **btrfs**[^btrfs-status]: 更新一代的文件系统. 提供先进特性如快照/通过校验和进行自我修复/透明压缩/子卷和集成 RAID.
...
[^btrfs-status]: btrfs 的[主要特性已经稳定](https://btrfs.readthedocs.io/en/latest/Status.html#overview). 越来越多的发行版已经将 btrfs 作为默认文件系统, 从最早的 SUSE Linux (2013), Fedora (2020) 到最近的 Manjaro (2025).

如果对 btrfs 兴趣不大, 只是想快速且简单地完成系统的安装, 建议使用 ext4.  
...
下面是参考 openSUSE (2018) 的子卷布局方案[^subvolume-layout]:
...
[^subvolume-layout]: <https://en.opensuse.org/SDB:BTRFS>
...
- `grub-btrfs`: 在 GRUB 引导菜单里添加 btrfs 快照入口 (允许从快照启动).[^grub-snapshot-entry]

[^grub-snapshot-entry]: <https://github.com/Antynea/grub-btrfs>
