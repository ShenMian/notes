...
下面将使用 fictx5 作为输入法框架 (Input Method Framework, IMF), 该框架有以下优点:
...
- `fcitx5-chinese-addons`: fictx5 官方开发的输入法引擎, 开箱即用.
...
```sh
sudo flatpak override com.tencent.WeChat --env=GTK_IM_MODULE=fcitx
sudo flatpak override com.tencent.WeChat --env=QT_IM_MODULE=fcitx
sudo flatpak override com.tencent.WeChat --env=XMODIFIERS=@im=fcitx
```
...
[薄荷输入法](https://github.com/Mintimate/oh-my-rime)提供了一套开箱即用的完整配置, 包括简体词库和皮肤.
...
[薄荷输入法皮肤](https://github.com/witt-bit/fcitx5-theme-mint)可以通过下面命令安装:
