...
原文链接: <https://mara.nl/atomics/atomics.html>.
...
但如果需要基于原子变量的值进行修改, 只使用 `load` 和 `store` 操作, 就可能导致**竞争**[^race-vs-data-race].  
...
[^race-vs-data-race]: 是竞争 (race) 而非数据竞争 (data race), 数据竞争是未定义行为, 不会出现在 safe Rust 中.
