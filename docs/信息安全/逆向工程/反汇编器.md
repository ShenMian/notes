...
!!! warning
    该项目早期版本在未修复的已知的远程可执行漏洞 [CVE-2021-44228](https://www.cve.org/CVERecord?id=cve-2021-44228), 详情请参考[安全警告](https://github.com/NationalSecurityAgency/ghidra#security-warning).
    请根据手册来进行补丁.  
...
- 部分键位和大多数软件并不相同, 具体请参考[快捷键](https://docs.hex-rays.com/core/user-interface/reference/shortcuts).
...
- [Cutter](https://github.com/rizinorg/cutter) Rizin 的 GUI, Fork 自 iaito.
...
支持 Time Travel Debugging (TTD)[^ttd].
...
- 可以对代码进行反汇编, 但只是简单地输出反汇编代码. 可以搭配 Radare2 使用 (LLDB 需要 r2lldb 插件), 同时具备调试和反汇编功能.
...
QIRA (QEMU Interactive Runtime Analyser) 是一款时间无关[^ttd]的调试器. 可以集成到 IDA Pro 中.
...
[^ttd]: <https://en.wikipedia.org/wiki/Time_travel_debugging>
